...
-- Employees earning more than their manager in the same department
SELECT 
    e.name AS employee_name,
    e.salary AS employee_salary,
    m.name AS manager_name,
...
-- Managers with more than 2 employees reporting
SELECT 
...
-- Employees + manager names sorted by manager salary (DESC)
SELECT 
    e.name AS employee_name,
    m.name AS manager_name,
    m.salary AS manager_salary
